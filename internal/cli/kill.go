...
For graceful shutdown, use 'cci shutdown' instead.

Use 'cci list' to see active containers.

Examples:
  cci kill claude-abc12345-1           # Force kill specific container
  cci kill claude-abc12345-1 claude-xyz78901-2  # Force kill multiple containers
  cci kill --all                       # Force kill all containers (with confirmation)
  cci kill --all --force               # Force kill all without confirmation
`,
	RunE: killCommand,
}
