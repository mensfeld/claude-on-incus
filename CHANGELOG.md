...
- **Switch tools on the same container via profiles (#708)** — re-enter a persistent container (keeping its code, installed packages, and running services) with a *different* AI tool, driven entirely by profiles rather than a flag. Two profiles that share `[container] session_name` (with `persistent = true`) resolve to the **same** container — its identity is `hash(workspace, session_name)` — so `coi shell --profile box-claude` creates the box and `coi shell --profile box-codex` re-enters that same box running codex. The enabling fix: on persistent **reuse**, `session.Setup` now seeds a tool's CLI config/credentials when that tool's config dir is empty/unseeded in the container (previously it always skipped seeding on reuse, so a newly-introduced tool had no auth) — targeted so it never re-copies the original tool's config or touches conversation history. (The "already seeded?" check keys off the tool's own essential config files, not the config-dir's existence or content: the base image pre-creates `~/.claude`/`~/.codex` empty and agent installers can leave unrelated files in a tool's dir, so an existence/content check would wrongly skip seeding — a bug the profile switch-on-reuse integration test caught.) (This replaces the short-lived `--tool` flag, which is reverted: tool choice is config/profile-shaped, and `coi shell --tool …` again points you at `[tool] name`.) Covered by a unit test for the reuse-seeding probe plus integration tests: a fresh `coi shell` (no `--resume`) reuses the stopped persistent container instead of forking a new slot, and a full claude→codex switch on a reused box seeds the new tool's config while leaving the original tool's config intact.
...
- **Config/profile symmetry: `mounts` blocks read the same in both scopes, and `env_command_timeout` is now settable per profile (#783)** — two consistency fixes so more config can live in profiles with no divergence. (1) A `mounts` block used to require a *different* TOML shape depending on scope — top-level config took the nested `[[mounts.default]]` form while a profile took the flat `[[mounts]]` form, so a mount block couldn't be copied between the two. Both scopes now accept **either** shape (a custom unmarshaler on `MountsConfig`/`MountList` decodes the flat array *or* the `{ default = [...] }` table, ignoring unknown keys and erroring loudly on a wrong-typed field), so the same block works verbatim in `~/.coi/config.toml`, a project `.coi/config.toml`, and any profile. (2) `[defaults] env_command_timeout` — the duration that bounds each `env_commands` invocation — had no profile counterpart, so a profile could define `env_commands` but not the timeout governing them; profiles now accept `env_command_timeout` too, applied and inherited alongside `env_commands` and, like `env_commands` themselves, stripped from untrusted (project-scoped) profiles. The profile JSON schema is updated for both (`mounts` accepts either shape — via `if/then/else` so an invalid entry in the array form still reports a precise `mounts[i]` error path; `env_command_timeout` added), and `coi profile info` now echoes a profile's `env_command_timeout`. A lone `env_command_timeout` from an untrusted (project-scoped) profile or config is stripped even when it carries no `env_commands`, so a cloned repo cannot shorten/extend the timeout applied to trusted-scope `env_commands`. Covered by Go unit tests (both mount shapes decode identically at top-level and profile scope, empty/type-error cases; timeout apply/inherit/untrusted-strip including the lone-timeout case) and integration tests (a real container mount via the flat top-level form; `coi profile info` accepting nested `[[mounts.default]]` and echoing `env_command_timeout`).

- **Consistent, case-insensitive container-status checks + a single source of truth for security device prefixes (robustness cleanup, #782)** — Incus reports lifecycle state as `Running`/`Stopped` via the JSON API but upper-case in some `incus list` formats; call sites had drifted between exact `== "Running"` comparisons and `strings.EqualFold`, so a status read from the "wrong" surface could be silently misclassified. All checks now go through shared `container.StatusIsRunning`/`StatusIsStopped` helpers. Separately, the security disk-device prefixes (`protect-`/`mask-`/`gitc-`) were three independent string literals on the create side that had to stay in sync with the reuse-strip list by hand; they're now shared constants, with a test (`TestSecurityDeviceNamesAreStripped`) asserting every device-name generator produces a name the strip list removes — so a new family can't leak across persistent reuse (the #610 class). No behavior change.
...
- **`coi build` no longer fails right after publishing on a non-default `[incus] project` (#777, thanks @marshalfevzi)** — after building and publishing the sandbox image, `getImageFingerprint` looked the freshly-published image up with a hardcoded `--project default`, so on any non-default `[incus] project` the lookup ran against the wrong project and the build aborted with "image not found: coi-default" — immediately after a *successful* publish into the configured project. The fingerprint lookup now uses `container.IncusProject` like every other image operation, so build works in any project. The lookup argv is built by a small `imageFingerprintListArgs` helper so the project wiring is guarded by a unit test (`TestImageFingerprintListArgs_UsesConfiguredProject`) that runs in the incus-less unit job, plus a best-effort integration test that builds in an isolated non-default project and asserts success (`tests/build/build_nondefault_project.py`).
...
- **`coi shell` under kitty (and other `xterm-*` terminals) no longer fails with "missing or unsuitable terminal" (#772)** — kitty exports `TERM=xterm-kitty`, but `SanitizeTerm` only matched a bare `kitty` prefix (a value kitty never sets), so `xterm-kitty` fell through unmapped; the container base image has no `xterm-kitty` terminfo, so tmux aborted the session. `SanitizeTerm` now maps *any* non-standard `xterm-*` value (and the emulator-specific names `foot`/`rio`/`contour`/`st-…` alongside the existing wezterm/alacritty/ghostty set) to `xterm-256color`, so a new emulator's `xterm-<vendor>` is handled without another code change. Two test layers guard it: the unit tests now assert the TERM values terminals **actually export** (`xterm-kitty`, `foot`, …) rather than invented ones — the gap that let this ship — and a new container integration test (`tests/shell/ephemeral`) launches a real session with `TERM=xterm-kitty` and asserts the shell starts with no terminal error, which the host-side Go tests structurally can't catch (they run against the host's terminfo, where a kitty user's machine *has* `xterm-kitty`). Workaround `env TERM=xterm-256color coi shell` is no longer needed.
