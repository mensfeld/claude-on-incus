"""
Test for cci tmux send - error when container is stopped.

Tests that:
1. Launch a container
2. Stop the container
3. Try to send command via tmux send
...
def test_tmux_send_stopped_container(coi_binary, cleanup_containers, workspace_dir):
    ...
    # Should fail
    assert result.returncode != 0, "Tmux send should fail for stopped container"
