...
def test_publish_and_delete_image(coi_binary, cleanup_containers, workspace_dir):
    ...
    result = subprocess.run(
        [coi_binary, "image", "publish", container_name, test_image_name,
         "--description", "Test image for integration tests"],
        capture_output=True,
        text=True,
        timeout=120,
    )
    assert result.returncode == 0, \
        f"Image publish should succeed. stderr: {result.stderr}"
    ...
    assert result.returncode == 0, \
        f"Published image should exist. stderr: {result.stderr}"
    ...
    assert result.returncode == 0, \
        f"Image delete should succeed. stderr: {result.stderr}"
    ...
    assert result.returncode != 0, \
        f"Deleted image should not exist. stdout: {result.stdout}"
