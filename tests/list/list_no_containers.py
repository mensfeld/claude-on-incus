"""
Test for cci list - no containers running.

Tests that:
1. Run cci list when no containers exist
...
def test_list_no_containers(coi_binary, cleanup_containers):
    """
    Test cci list when no containers are running.
    ...
    assert "(none)" in output or "cci-" in output or "Status:" in output, (
